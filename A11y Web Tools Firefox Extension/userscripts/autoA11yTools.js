(async () => {
    ...
    const excludedPaths = [
        /^https:\/\/[^/]+\.instructure\.com\/courses\/1026(\/|$)/, // Training course
        /^https:\/\/[^/]+\.instructure\.com\/courses\/\d+\/modules$/, // Any course's modules page
        /^https:\/\/[^/]+\.instructure\.com\/courses\/\d+\/(pages|assignments|quizzes)\/[^/]+\/edit(?:[?#].*)?$/, // Any course's edit view
        /^https:\/\/[^/]+\.instructure\.com\/courses\/\d+\/files(?:\/.*|\?.*)?$/, // Any course's files page or subfolder
        /^https:\/\/[^/]+\.instructure\.com\/courses\/?$/, // Canvas courses page
        /^https:\/\/[^/]+\.instructure\.com\/?$/, // Canvas main page
        /^https:\/\/[^/]+\.instructure\.com\/calendar(?:\/.*|[#?].*)?$/, // Canvas calendar page
        /^https:\/\/[^/]+\.instructure\.com\/conversations(?:\/.*|[#?].*)?$/, // Canvas inbox page
        /^https:\/\/[^/]+\.instructure\.com\/courses\/\d+\/?$/, // Any course's home page
    ...
    // Central tool tracking object (ordered by loading order)
    const TOOLS = {
        HEADING: {
            id: "heading",
            label: "Heading Tags",
    ...
    function runContrastHighlights(container) {
        ...
        function scanContrast() {
            ...
            while (walker.nextNode()) {
                ...
                function copyBackgroundColor(e) {
                    if (!e.shiftKey) return;
                    if (newColors.background) {
